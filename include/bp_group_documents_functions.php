...
function bp_group_documents_check_ext( $filename ) {
	...
	/**
	*@since 1.22
	*we use trim() to avoid spaces in the $valid_formats
	*/
	$valid_formats_array  = array_map( 'trim', $valid_formats_array );
...
function bp_group_documents_register_taxonomies() {
	register_taxonomy(
		...
		array(
			'hierarchical' => true,
			'label'        => __( 'Group Document Categories', 'bp-group-documents' ),
			'query_var'    => false,
		)
	);
}
