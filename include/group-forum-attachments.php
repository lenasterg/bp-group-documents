...
function bp_group_documents_forum_attachments_upload_attachment()
{
    ?>
    <div>
        <a id="bp_group_documents_forum_upload_toggle" href="#"><?php esc_html_e('Upload document (+)', 'bp-group-documents'); ?></a>
    </div>
...
function bp_group_documents_forum_attachments_topic_text( $topic_text )
{
    ...
    if (! empty($_FILES) ) {
        ...
        $document->description = sanitize_textarea_field(wp_unslash($_POST['bp_group_documents_description']));
        if ($document->save() ) {
...
{
    $html = "<br /><a class='group-documents-title' id='group-document-link-{$document->id}' href='".esc_url($document->get_url())."' target='_blank'>".esc_html($document->name);
