...
?>
	...
	if ( $document->current_user_can( 'edit' , $group_id ) ) {
	    $can_edit = true ;
	}

	if ( $document->current_user_can( 'delete' , $group_id ) ) {
	    $can_delete = true ;
	}
    ?>
	                <li 
...
if ( $template->show_detail ) {
	...
	?>
		...
		<?php if ( $template->doc_in_category( $category->term_id ) ) {
		    echo 'checked="checked"' ;
		}
		?> 
		                />
